  ...
  {"id":"bf-10","name":"Porridge","category":"Breakfast","image":"https://images.unsplash.com/photo-1518047601542-79f18c655718?w=500","time":"12 min","ingredients":["Oats","Water","Salt"],"instructions":["Cook oats in water","Simmer","Add toppings"],"addedBy":"system"}
        ...
        function loadRecipes() {
            const storedRecipes = JSON.parse(localStorage.getItem('recipeMasterRecipes')) || [];
        ...
        function initApp() {
            ...
            if (isAdmin) {
                ...
                elements.addRecipeBtn.style.display = 'block';
            } else {
                document.getElementById('adminBadge').style.display = 'none';
                document.getElementById('adminControls').style.display = 'none';
                elements.addRecipeBtn.style.display = 'none';
            }
        ...
        function renderRecipes(recipes) {
            ...
            if (recipes.length === 0) {
                ...
                recipes.forEach(recipe => {
                    ...
                    const isAdmin = currentUser && (currentUser.isAdmin || currentUser.email === ADMIN_EMAIL);
                    
                    card.innerHTML = `
                        ${isAdmin ? '<button class="delete-btn" onclick="deleteRecipe(\'' + recipe.id + '\', event)">Delete</button>' : ''}
